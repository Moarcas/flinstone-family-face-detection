...
class FacialDetector:
        ...
        for name in ['barney', 'betty', 'fred', 'wilma']:
            ...
            with open(annotations_file, 'r') as f:
                ...
                for line in lines:
                    ...
                    if self.params.face_image_dimension[0] - crop_image.shape[0] <= 0 or self.params.face_image_dimension[1] - crop_image.shape[1] <= 0:
                        continue
                    ...
                    face_image = cv.copyMakeBorder(crop_image,
                                                   top=0,
                                                   bottom=self.params.face_image_dimension[0] - crop_image.shape[0],
                                                   left=0,
                                                   right=self.params.face_image_dimension[1] - crop_image.shape[1],
                                                   borderType=cv.BORDER_REPLICATE)
                    cv.imwrite(face_file, face_image)

    def find_negative_images(self, image_path, characters_positions):
        image = cv.imread(image_path, cv.IMREAD_GRAYSCALE)
        iou_threshold = 0.1
        for ratio in self.params.ratio_window:
            resized_image = cv.resize(image, (int(image.shape[0] * ratio), int(image.shape[1] * ratio)))
            start_row_window = np.random.randint(low=0, high=resized_image.shape[0] - self.params.face_image_dimension[0] + 1)
            start_column_window = np.random.randint(low=0, high=resized_image.shape[1] - self.params.face_image_dimension[0] + 1)
            final_row_window = start_row_window + self.params.face_image_dimension[0] - 1
            final_column_window = start_column_window + self.params.face_image_dimension[0] - 1
            ...
            for character_position in characters_positions:
                new_character_position = [int(ratio * position) for position in character_position]
                iou_window = self.intersection_over_union(new_character_position, window_position)
                if iou_window != 0:
                    good_window = False
                    break

            if good_window:
                window = resized_image[start_row_window:(final_row_window + 1), start_column_window:(final_column_window + 1)]
    ...
    def run(self):
        ...
        for i in range(10):
            ...
            for ratio in self.params.ratio_window:
                resized_image = cv.resize(img, (int(img.shape[0] * ratio), int(img.shape[1] * ratio)))
                for start_row_window in range(0, resized_image.shape[0] - self.params.face_image_dimension[0] + 1, 20):
                    for start_column_window in range(0, resized_image.shape[1] - self.params.face_image_dimension[0] + 1, 20):
                        final_row_window = start_row_window + num_row_window
                        final_column_window = start_column_window + num_column_window
                        window = resized_image[start_row_window:final_row_window, start_column_window:final_column_window]
